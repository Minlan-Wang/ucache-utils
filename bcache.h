...
do {												\
	struct bkey *_k = xset->start;					\
	while (_k < bkey_idx(xset->start, xset->keys)) {	\
		dump_bkey(_k);								\
		_k = bkey_next(_k);							\
	}												\
} while (0)

#define destroy_xset_list(head, type)				\
do {												\
	type *_j;									\
	type *_i;									\
	list_for_each_entry_safe(_j, _i, head, list) {	\
		list_del(&_j->list);						\
		free(_j);									\
	}												\
} while(0)
...
#if 0
#define dump_set_list_bkeys(head, type, print)		\
do {												\
		type *_j;									\
		struct bkey *k;								\
		list_for_each_entry(j, head, list) {		\
			print();								\
			k = j->jset->start;						\
			while (k < bkey_idx(j->jset->start, j->jset->keys)) {	\
				dump_bkey(k);						\
				k = bkey_next(k);					\
			}										\
		}											\
